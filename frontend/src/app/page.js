...
const fallbackAgents = [
  { name: "OCR Agent", description: "Extract text from images and documents.", category: "documents", pricePerRequest: 0.01 },
  { name: "Summary Agent", description: "Turn extracted document text into a clear brief.", category: "language", pricePerRequest: 0.01 },
  { name: "Fraud Agent", description: "Detect suspicious invoice patterns and payment risk.", category: "security", pricePerRequest: 0.01 },
];
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([apiGet("/api/agents"), apiGet("/api/workflows")])
    ...
    <AppShell eyebrow="DASHBOARD">
      ...
      <section className="mt-10">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {visibleAgents.slice(0, 3).map((agent) => <Link href="/workflow" key={agent._id || agent.name} className="group rounded-2xl border border-white/10 bg-[#0b1b2d] p-5 transition hover:-translate-y-1 hover:border-cyan-300/40"><div className="flex items-start justify-between"><span className="rounded-full bg-cyan-300/10 px-2.5 py-1 text-[11px] uppercase tracking-wider text-cyan-200">{agent.category}</span><StatusPill status="Online" /></div><h3 className="mt-8 text-lg font-semibold text-white">{agent.name}</h3><p className="mt-2 min-h-12 text-sm leading-5 text-slate-400">{agent.description}</p><p className="mt-6 text-sm text-slate-300">${Number(agent.pricePerRequest || 0.01).toFixed(2)} <span className="text-slate-500">per request</span></p></Link>)}
        </div>
      </section>
